<template>
  <div id='alcohol'>
    ...
    <el-table id="alcoholTable" :data="alcoholData" border style="width:100%;" stripe size="mini" @row-dblclick='lookInfoForm'>
      ...
      <el-table-column prop="alcoholDtlTime" :label="$t('common.upTime')"></el-table-column> <!-- 上传时间 -->
        ...
        <el-row>
          ...
          <el-col :span='12'>
            <el-form-item :label="$t('alcohol.picturesForAlcoholTesting')">
              <viewer :images="[infoForm.picturesForAlcoholTesting]" v-if="infoForm.picturesForAlcoholTesting">
                <img :src="infoForm.picturesForAlcoholTesting" width="275" style="cursor:pointer;" height="200">
              </viewer>
            </el-form-item>
          </el-col>
          <el-col :span='12'>
...
export default {
  ...
  methods: {
    ...
    lookInfoForm(row){
      ...
      this.infoForm.picturesForAlcoholTesting = row.alcoholDtlImage;
      ...
      this.infoForm.upTime = row.alcoholDtlTime;
    ...
    setExcel(dataList){
      if(dataList&&dataList.length){
        require.ensure([], () => {
          ...
          const tHeader = [this.$t('device.deviceName'),this.$t('device.deviceHardwareId'),this.$t('deviceu.deviceuName'),this.$t('devicel.devicelName'),this.$t('device.deviceType'),this.$t('deviceu.deviceuImage'),this.$t('alcohol.picturesForAlcoholTesting'),this.$t('alcohol.proof'),this.$t('alcohol.confidenceOfcomparison'),this.$t('alcohol.alcoholCountry'),this.$t('alcohol.alcoholUnit'),this.$t('common.upTime')]; //这个是表头名称 可以是iveiw表格中表头属性的title的数组
          const filterVal = ['deviceName','deviceHardwareId','deviceuName','devicelName','deviceType','deviceuImage','alcoholDtlImage','alcoholDtlValue','alcoholStandard','alcoholCountry','alcoholUnit','alcoholDtlTime']; //与表格数据配合 可以是iview表格中的key的数组
          const list = dataList; //表格数据，iview中表单数据也是这种格式！
          const data = this.formatJson(filterVal, list)
          export_json_to_excel(tHeader, data, this.$t('header.subCol.alcohol')+' '+this.curentTime()) //列表excel  这个是导出表单的名称
        })
      }
    },
    nowExcel(){
      this.setExcel(this.alcoholData)
    },
    allExcel(){
      this.$axios({
        method: "post",
        url: NEW_URL_CONFIG + "/api/viewdata/alcoholrelate",
        headers: {
        ...
        if (res.data.statusCode == 200) {
          ...
          res.data.data.resultObjects.forEach((val) => {
            dataList.push({
              ...
              alcoholDtlImage: val.alcoholDtlImage,
              alcoholDtlTime: val.alcoholDtlTime.replace('T',' '),
              alcoholDtlValue: val.alcoholDtlValue,
              alcoholStandard: val.alcoholStandard,
    ...
    getAlcoholData(pageIndex,pageSize,deviceId,deviceType,alcoholStartTime,alcoholEndTime){
      this.$axios({
        method: "post",
        url: NEW_URL_CONFIG+"/api/viewdata/alcoholrelate",
        headers: {
        ...
        if(res.data.statusCode == 200){
          ...
          res.data.data.resultObjects.forEach((val)=>{
            this.alcoholData.push({
              ...
              alcoholDtlValue: val.alcoholDtlValue,
              alcoholStandard: val.alcoholStandard,
              alcoholTwoLevel: val.alcoholTwoLevel,
              alcoholThreeLevel: val.alcoholThreeLevel,
              alcoholUnit: val.alcoholUnit,
              deviceType: val.deviceType,
              devicelName: val.devicelName,
              deviceuImage: val.deviceuImage,
    ...
    onSubmit() {
      ...
      this.getAlcoholData(this.currentPage,this.pageTotal,this.dateQuery.deviceId,this.dateQuery.deviceType,start,end);
    },
    //切换页码
